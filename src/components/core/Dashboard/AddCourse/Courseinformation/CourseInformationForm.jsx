...
const CourseInformationForm = () => {
    ...
    const onSubmit = async(data) => {
        ...
        setLoading(true);
        console.log("Before add course API call token ", token)
        console.log("BEFORE add course API call");
        console.log("PRINTING FORMDATA", formData);
       //yaha dikkat hai kuch 
        const result = await addCourseDetails(formData,token);
        console.log("After API call token ", token)
        if(result) {
            console.log("mai result hu bhaiya",result)
            dispatch(setStep(2));
            dispatch(setCourse(result));
        }
        setLoading(false);
